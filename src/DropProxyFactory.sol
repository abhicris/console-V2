...
import {IDropProxyFactory} from "./interfaces/IDropProxyFactory.sol";

/**
 * @title DropProxyFactory
 * @notice A factory that deploys minimal proxies of `ERC721ConsoleDrop.sol`.
 * @dev The proxies are OpenZeppelin's Clones implementation of https://eips.ethereum.org/EIPS/eip-1167
 */

contract DropProxyFactory is IDropProxyFactory, OwnableRoles {
    /**
     * @dev The implementation contract delegated to by Drop proxies.
     */
    address public dropImplementation;

    constructor(address _dropimplementation) {
    ...
    function createDrop(
        ...
        assembly {
            ...
            if iszero(
                call(
                    gas(), // Gas remaining.
                    consoleDrop, // Address of the edition.
        ...
        OwnableRoles(consoleDrop).transferOwnership(msg.sender);
    ...
    function setNewImplementation(address newImplementation)
        external
        onlyOwner
    {
